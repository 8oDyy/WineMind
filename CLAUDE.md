...
- **Reste légitimement côté client (Supabase direct)** : Supabase Auth uniquement — `register`/`login`/`logout`/`getCurrentUser`/`signInWithGoogle` dans `auth_remote_data_source.dart`, et le refresh de session dans `app.dart`.

### Google Sign-In (flow natif iOS — `signInWithIdToken`)

- **Mécanisme** : `google_sign_in` ^7.1.0 (nouvelle API v7 : `GoogleSignIn.instance.initialize()` + `.authenticate()` + `authorizationClient`) → on récupère l'`idToken` + l'`accessToken` Google, puis `supabase.auth.signInWithIdToken(provider: OAuthProvider.google, ...)`. **Pas de flow redirect/deeplink** (évite la plomberie URL scheme PKCE).
- **Nonce — choix : AUCUN nonce côté Flutter** (contrat aligné avec Supabase). Sur iOS, `google_sign_in` v7 ne garantit pas qu'un nonce custom corresponde au hash de l'`idToken` renvoyé, donc on n'en passe pas (et pas de dépendance `crypto`). ⚠️ **Contrat back↔front** : le provider Google de Supabase **doit** être configuré avec **« Skip nonce checks » = ON**, sinon le token est rejeté. Les deux côtés sont cohérents : pas de nonce ici / vérif nonce désactivée là-bas (= exemple officiel Supabase pour le flow natif Flutter). Décision documentée en commentaire dans `signInWithGoogle()`.
- **Chaîne Clean Archi** : `signInWithGoogle()` traverse datasource → `AuthRepository` (contrat) → `AuthRepositoryImpl` (mappe `GoogleSignInException`/`AuthException` → `AuthFailure`, l'annulation utilisateur devient un message neutre) → usecase `SignInWithGoogle` (`NoParams`) → handler `on<LoginWithGoogleEvent>` dans `AuthBloc` → DI `injection_container.dart`. Les 2 boutons (`login_page`, `create_account_page`) dispatchent `LoginWithGoogleEvent` et sont désactivés en `AuthLoading`.
- **Client IDs** : RENSEIGNÉS dans `AppConfig.googleIosClientId` / `googleWebClientId` (clients OAuth Google Cloud, bundle `fr.winemind.app`). `AppConfig.isGoogleSignInConfigured` renvoie désormais `true`. Le Web client ID = `serverClientId` attendu par Supabase comme audience.
- **Config native iOS** : `ios/Runner/Info.plist` contient `GIDClientID` (iOS client ID) + `CFBundleURLTypes` avec le reversed client ID — **actif** (plus en commentaire). `plutil -lint` OK.
- **Dernier prérequis restant** : provider Google activé côté **Supabase** (Auth > Providers > Google : Enable + Web client ID + Web secret + **Skip nonce checks = ON**) — action **manuelle dashboard utilisateur**. Tant que ce n'est pas fait, le bouton ouvre la popup Google mais le retour de session Supabase échouera. Guide utilisateur : `docs/google-sign-in-setup.md`.
- **Bundle ID unifié** : `fr.winemind.app` (Runner, toutes configs Debug/Profile/Release) + `fr.winemind.app.RunnerTests` (cible de tests). Décision utilisateur : app **pas encore publiée**, donc les anciens identifiants incohérents (`com.example.winemind` en Debug/Profile, `com.boulicaut.winemind` en Release) ont tous été remplacés. C'est ce bundle ID qui doit être déclaré dans le client OAuth **iOS** Google Cloud.
